...
7. **上下文可接续**：任何 Agent 都能从固定入口理解代码层级、长期规范、当前工作和未来计划。
...
所有 coding agent 使用同一个仓库内上下文模型，不依赖 Claude、Codex、Cursor 或其他工具的私有会话记忆：

| 需要理解的内容 | 事实来源 | 维护时机 |
|---|---|---|
| 项目是什么、边界在哪里 | `PROJECT_SPEC.md` | 产品方向或技术边界变化时 |
| 代码层级、目录和关键文件职责 | `FILE_MAP.md` | 重要文件新增、移动、删除或职责变化时 |
| 工程、UI、命名和架构规范 | `STYLE_GUIDE.md`、`DECISIONS.md` | 规范或长期决策变化时 |
| 正在进行的工作、阻塞和下一动作 | `HANDOFF.md` | 开始工作、出现阻塞、暂停或完成交接时 |
| 所有 spec 的状态和依赖 | `SPEC_INDEX.md` | 每次 spec 状态或关系变化时 |
| 未来计划和优先级 | `ROADMAP.md` | 计划进入 now/next/later 或完成时 |

`HANDOFF.md` 是固定的短入口，包含当前目标、活跃 spec、工作状态、最近验证、阻塞项、未提交风险和下一动作，并链接到上述事实文件。它不复制详细规范。`ROADMAP.md` 使用 `now`、`next`、`later`、`done` 四个区段，每项关联 spec 或明确标记为尚未规格化的 idea。

跨工具薄入口 `CLAUDE.md`、`AGENTS.md` 等只要求先读 `.vibe-spec/HANDOFF.md` 和 `.vibe-spec/AGENT_GUIDE.md`。Agent 接手顺序固定为：交接摘要 -> 项目和规范 -> 活跃 spec -> 相关代码与测试。

Skill 在每次工作开始时执行 context check，在结束、暂停或切换 Agent 前执行 handoff update。`refresh_context.py` 可扫描顶层目录、语言清单和常见构建入口，生成 `FILE_MAP.md` 的候选差异；候选必须由 Agent 核对后合并，避免把生成目录或临时文件写成长期架构事实。
...
初始化完成后，Agent 必须补齐最小接手信息：项目摘要、代码主入口、测试入口、当前工作和下一步。无法从仓库确认的内容明确标为 `unknown` 并向用户确认，不凭空推断。
...
`status_vibe_spec.py` 从 spec、`HANDOFF.md` 和 `ROADMAP.md` 重建状态视图，展示状态分布、当前工作、阻塞项、待同步项、近期计划和建议下一步。`check_vibe_spec.py` 扩展索引一致性、继承引用、重复 ID、生命周期证据、上下文新鲜度和模块配置检查。
...
- 固定接手入口、当前工作和 roadmap 的初始化与更新
- 文件地图扫描只生成候选，不覆盖人工维护内容
- spec 创建、重复 ID 和命名规范
- 合法与非法状态流转
- 各生命周期门禁失败和成功
- spec、生命周期日志、索引三者同步
...
完成标准：Claude 风格命令和 Codex 自然语言调用都有明确路径；任意 Agent 能从固定入口理解代码层级、规范、当前工作和未来计划；核心生命周期可以自动校验和同步；生产增强项可选启用；测试、严格检查和独立审查通过。
